...
function buildRawPlan(modules, viewport, prevMembership) {
  ...
  return { chains, tonality, membership };
}

// ---- stateful debounce ----
let _committed = { chains: [], tonality: null, membership: new Set() };
let _holds = {};
...
function update(modules, viewport) {
  ...
  _committed = { chains: newChains, tonality: raw.tonality, membership: raw.membership };
  return _committed;
}

function reset() {
  _committed = { chains: [], tonality: null, membership: new Set() };
  _holds = {};
}
